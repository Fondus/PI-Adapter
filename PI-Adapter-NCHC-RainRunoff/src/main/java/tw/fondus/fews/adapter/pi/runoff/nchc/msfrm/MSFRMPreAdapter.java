...
public class MSFRMPreAdapter extends RainRunoffPreAdapter {
	...
	protected String createModelInputContent( TimeSeriesArray array ) {
		...
		IntStream.range(0, array.size()).forEach(i -> {
			if ( i == 0 ) {
				content.add(Strman.append( String.valueOf(i + 1), "\t", String.valueOf( TimeSeriesLightUtils.getValue(array, i, 0.0F) ), "\t", "! TIME ; OBSERVED RAINFALL"));
			} else {
				content.add(Strman.append( String.valueOf(i + 1), "\t", String.valueOf( TimeSeriesLightUtils.getValue(array, i, 0.0F) ) ));
			}
		});
